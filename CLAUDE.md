...
- Before committing: `node tools/stamp.mjs` (writes `version.json` and
  `src/version.js`; the deployed app uses them to refresh GitHub Pages' 10 min
  cache on the phone; also writes `docs/files.json`, the manifest the app's
  "Load from site" buttons read) and `node test/run.mjs` (must print ALL OK).
- New CSV export: drop it in `docs/` (delete the older file of the same
  kind), run the stamp, push. The phone can then load it from the site.
...
- Icons are generated by `node tools/make-icons.mjs` (no dependencies).
- `node tools/plan.mjs [leagueId]` prints the pre-draft plan (tiers per
  position, expected best available at each of the user's turns, tier
  drain) from the computer; the app's Board has the same view.
